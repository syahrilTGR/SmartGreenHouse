...
BLYNK_CONNECTED() {
  ...
  Blynk.syncVirtual(V4); // Minta status LED terakhir dari server Blynk
}

// Fungsi ini dipanggil otomatis ketika tombol (Datastream V4) di aplikasi Blynk ditekan
BLYNK_WRITE(V4) {
  int pinValue = param.asInt(); // Ambil nilai dari tombol (0 atau 1)
  lastLampState = (pinValue == 1); // Perbarui variabel state
  digitalWrite(RELAY_LAMP_PIN, lastLampState ? HIGH : LOW); // Nyalakan/Matikan Relay Lampu
  Serial.printf("=> Aksi Blynk: Lampu %s\r\n", lastLampState ? "Dinyalakan" : "Dimatikan");
}
